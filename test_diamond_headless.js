// ============================================================
// Headless Test for Diamond Movement Pattern
// Moves in a diamond: northeast, southeast, southwest, northwest
// ============================================================
...
const strategyCode = fs.readFileSync(path.join(__dirname, 'src/strategies/attacker_diamond.js'), 'utf-8');
...
function testDiamondPattern() {
  ...
  console.log('Testing Diamond Movement Pattern (using real PhysicsEngine)');
  console.log('='.repeat(80));
  ...
  for (let frame = 0; t_ms < duration_ms; frame++) {
    ...
    if (motorChanged && timeElapsed && steps.length < 10 && lastStepTransitionTime > 0) {
      ...
      if (stepDistance > 0.3) {
        ...
        // Diagonal directions - check if both components are significant
        // Use a ratio check: if both dx and dy are at least 30% of the larger component, it's diagonal
  ...
  if (finalStepDistance > 0.3 && steps.length < 10) {
    const totalDx = posX - stepStartPosX;
    const totalDy = posY - stepStartPosY;
    const dxAbs = Math.abs(totalDx);
    const dyAbs = Math.abs(totalDy);
  ...
  const expectedSteps = ['northeast', 'southeast', 'southwest', 'northwest'];
  ...
  if (passed) {
    console.log('\n✅ TEST PASSED: Diamond pattern is correct!');
  } else {
...
try {
  const result = testDiamondPattern();
